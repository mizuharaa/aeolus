...
SAMPLE_SCHEDULE = [
    {"id": "NB101", "aircraft_id": "N001NB", "origin": "KORD", "destination": "KATL",
     ...
     "passengers": 148, "status": "scheduled"},
    {"id": "NB102", "aircraft_id": "N001NB", "origin": "KATL", "destination": "KMIA",
     ...
     "passengers": 135, "status": "scheduled"},
    {"id": "NB103", "aircraft_id": "N002NB", "origin": "KORD", "destination": "KDFW",
     ...
     "passengers": 150, "status": "scheduled"},
    {"id": "NB104", "aircraft_id": "N002NB", "origin": "KDFW", "destination": "KLAX",
     "scheduled_departure": "2024-01-15T17:45:00Z", "scheduled_arrival": "2024-01-15T20:00:00Z",
     "passengers": 142, "status": "scheduled"},
]
...
class TestWeatherClosure:
    def test_kind(self):
        ev = _make_event(WeatherClosureEvent, {"airport": "KORD", "severity": "severe", "duration_hours": 4})
    ...
    def test_affected_flights_not_at_other_airports(self):
        ev = _make_event(WeatherClosureEvent, {"airport": "KMSP", "severity": "severe", "duration_hours": 4})
        affected = ev.affected_flights(SAMPLE_SCHEDULE)
...
class TestAirspaceClosure:
    def test_kind(self):
        ev = _make_event(AirspaceClosureEvent, {
            "polygon": {"type": "Polygon", "coordinates": [[[44, 25], [63, 25], [63, 40], [44, 40], [44, 25]]]},
            "duration_hours": 24,
        })
...
class TestSecurityEvent:
    def test_kind(self):
        ev = _make_event(SecurityEvent, {"airport": "KATL", "severity": "severe", "duration_hours": 3})
        assert ev.kind == EventKind.SECURITY_EVENT

    def test_affects_all_at_airport(self):
        ev = _make_event(SecurityEvent, {"airport": "KORD", "severity": "severe", "duration_hours": 3})
        affected = ev.affected_flights(SAMPLE_SCHEDULE)
        ids = [f["id"] for f in affected]
        assert "NB101" in ids
...
class TestMechanicalAOG:
    def test_kind(self):
        ev = _make_event(MechanicalAOGEvent, {"aircraft_tail": "N001NB", "airport": "KATL", "duration_hours": 8})
    ...
    def test_does_not_affect_other_aircraft(self):
        ev = _make_event(MechanicalAOGEvent, {"aircraft_tail": "N999NB", "airport": "KORD", "duration_hours": 4})
        affected = ev.affected_flights(SAMPLE_SCHEDULE)
        assert len(affected) == 0
...
class TestCrewSickout:
    def test_kind(self):
        ev = _make_event(CrewSickoutEvent, {"base": "KORD", "percent_affected": 30, "duration_hours": 8})
...
class TestRunwayClosure:
    def test_kind(self):
        ev = _make_event(RunwayClosureEvent, {
...
class TestATCStaffing:
    def test_kind(self):
        ev = _make_event(ATCStaffingEvent, {"sector_or_airport": "KLAS", "capacity_pct": 40, "duration_hours": 5})
...
class TestVolcanicAsh:
    def test_kind(self):
        ev = _make_event(VolcanicAshEvent, {
            "polygon": {"type": "Polygon", "coordinates": [[[-125, 44], [-117, 44], [-117, 50], [-125, 50], [-125, 44]]]},
            "duration_hours": 18,
            "severity": "severe",
        })
...
class TestCyberIncident:
    def test_kind(self):
        ev = _make_event(CyberIncidentEvent, {"airline": "NimbusAir", "degradation_pct": 60, "duration_hours": 12})
